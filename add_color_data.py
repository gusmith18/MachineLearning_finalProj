...
# Pre-compute color ranges to avoid recreating them per image
COLOR_RANGES_HSV = {
    'red': (np.array([0, 100, 100], dtype=np.uint8), np.array([10, 255, 255], dtype=np.uint8)),
    'green': (np.array([40, 100, 100], dtype=np.uint8), np.array([80, 255, 255], dtype=np.uint8)),
    ...
    'black': (np.array([0, 0, 0], dtype=np.uint8), np.array([180, 255, 30], dtype=np.uint8)),
    'white': (np.array([0, 0, 200], dtype=np.uint8), np.array([180, 20, 255], dtype=np.uint8))
}
...
def color_detect(image):
    ...
    color_counts = {}
    for color, (lower_np, upper_np) in COLOR_RANGES_HSV.items():
...
def append_color_to_csv(csv_file, image_dir=None, name_column='name', image_url_column='image_url', output_file=None, chunk_size=50):
    ...
    print(f"\n{'='*60}")
    print(f"Starting: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(overall_start))}")
    print(f"{'='*60}\n")
    ...
    for col in color_columns:
        df[col] = np.nan  # Use np.nan for efficiency
    ...
    if image_dir and os.path.exists(image_dir):
        ...
        for i, image_path in enumerate(tqdm(images, desc="Processing local images")):
        ...
        print(f"\nLocal processing completed in {stage_elapsed:.1f}s")
        ...
        if len(df_to_process) == 0:
            ...
            with Pool(num_workers) as pool:
                all_results = list(tqdm(pool.imap_unordered(_process_url_chunk, chunks), 
                                       total=len(chunks), desc="Downloading & processing images"))
        ...
        stage_elapsed = time.time() - stage_start
        print(f"\nURL processing completed in {stage_elapsed:.1f}s")
    ...
    print(f"\n{'='*60}")
    print(f"Completed: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
    print(f"{'='*60}")
    print(f"Processed {processed} images")
    print(f"Matched color data to {matched} rows")
    print(f"Skipped {skipped} images")
    print(f"Total elapsed time: {overall_elapsed:.1f}s ({overall_elapsed/60:.1f} min)")
    if processed > 0:
        avg_per_image = overall_elapsed / processed
        print(f"Average time per image: {avg_per_image:.2f}s")
